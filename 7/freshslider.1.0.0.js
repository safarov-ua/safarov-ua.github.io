...
(function($){
    ...
    $.fn.freshslider = function(options){
        ...
        var updateCarets = function(){
            var value = ((round(values[1] * gap) + min).toFixed(countPoint) + unit);
            var lastValueNumber = value.substr(value.length-1,1);
            var dayName;
            /*---variation name of day----*/
            if(value > 9 && value < 21){
                dayName = " дней";
            }
             else if(lastValueNumber == 1){
                dayName =  " день";
            }
            else if(lastValueNumber == 2 || lastValueNumber == 3 || lastValueNumber == 4){
                dayName = " дня";
            }
            else{
                dayName =  " дней";
